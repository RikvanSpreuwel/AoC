...
int main()
{
    ...
    if (myfile.is_open())
    {
        ...
        for (auto sequence : sequences) {
            ...
            int newSequenceValue = subsequences[0][0];
            int differencePrediction = 0;
            for (int i=subsequences.size()-1;i>0;i--) {
                differencePrediction = subsequences[i][0]-differencePrediction;
                // newSequenceValue += subsequence.back();
            }
            sum += newSequenceValue-differencePrediction;
        }
