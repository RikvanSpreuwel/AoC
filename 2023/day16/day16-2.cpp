...
int main(){
    ...
    int maxNrEnergizedCells = 0;
    int minCellsEnergized = 10000;
    ...
    // test all top-side cells going down
    for (int i=0; i<grid[0].size();i++) {
        exploreBeam(0,i, Down);
        int nrEnergizedCells = countNrEnergizedCellsAndRefreshGrid();
        if (nrEnergizedCells > maxNrEnergizedCells) maxNrEnergizedCells = nrEnergizedCells;
        if (nrEnergizedCells < minCellsEnergized) minCellsEnergized = nrEnergizedCells;
    }

    // test all bottom-side cells going up
    for (int i=0; i<grid[0].size();i++) {
        exploreBeam(grid.size()-1,i, Up);
        int nrEnergizedCells = countNrEnergizedCellsAndRefreshGrid();
        if (nrEnergizedCells > maxNrEnergizedCells) maxNrEnergizedCells = nrEnergizedCells;
        if (nrEnergizedCells < minCellsEnergized) minCellsEnergized = nrEnergizedCells;
    }

    // test all left-side cells going right
    for (int i=0; i<grid.size();i++) {
        exploreBeam(i,0, Right);
        int nrEnergizedCells = countNrEnergizedCellsAndRefreshGrid();
        if (nrEnergizedCells > maxNrEnergizedCells) maxNrEnergizedCells = nrEnergizedCells;
        if (nrEnergizedCells < minCellsEnergized) minCellsEnergized = nrEnergizedCells;
    }

    // test all right-side cells going left
    for (int i=0; i<grid.size();i++) {
        exploreBeam(i,grid[0].size()-1, Left);
        int nrEnergizedCells = countNrEnergizedCellsAndRefreshGrid();
        if (nrEnergizedCells > maxNrEnergizedCells) maxNrEnergizedCells = nrEnergizedCells;
        if (nrEnergizedCells < minCellsEnergized) minCellsEnergized = nrEnergizedCells;
    }
    
    cout << maxNrEnergizedCells;
}